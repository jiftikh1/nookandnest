...
export default async function ItemsPage({ params }: { params: Promise<{ projectId: string; roomId: string }> }) {
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-12">
      ...
      <div className="flex border-b mb-10" style={{ borderColor: "#E0DCD6" }}>
        {subTabs.map((tab) => (
          <Link
            ...
            style={{
              borderColor: tab.href === `${base}/items` ? "#8B7355" : "transparent",
              color: tab.href === `${base}/items` ? "#8B7355" : "#7A7A7A",
            }}
          >
            {tab.label}
          </Link>
      ...
      <ItemsManager roomId={roomId} initialProducts={room.products} />
    </div>
  );
}
